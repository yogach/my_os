...
void PrintHeader(Fat12Header& rf, QString p)
{
    ...
    if( file.open(QIODevice::ReadOnly) )
    {
        ...
        qDebug() << "BPB_BytsPerSec: " << hex << rf.BPB_BytsPerSec; //使用16进制发送
...
RootEntry FindRootEntry(Fat12Header& rf, QString p, int i)
{
    ...
    if( file.open(QIODevice::ReadOnly) && (0 <= i) && (i < rf.BPB_RootEntCnt) )
    {
        ...
        file.seek(19 * rf.BPB_BytsPerSec + i * sizeof(RootEntry) ); //定位到目录文件区
...
RootEntry FindRootEntry(Fat12Header& rf, QString p, QString fn)
{
    ...
    {
        RootEntry re = FindRootEntry(rf, p, i); //读取对应的目录项

        if( re.DIR_Name[0] != '\0' )
        {
            int d = fn.lastIndexOf(".");
            QString name =  QString(re.DIR_Name).trimmed(); //去除前后的空格
...
{
    QFile file(p);
    int size = rf.BPB_BytsPerSec * 9; //fat表项的长度
    uchar* fat = new uchar[size];
    QVector<ushort> ret(size * 2 / 3, 0xFFFF); //设置大小和初始值
...
QByteArray ReadFileContent(Fat12Header& rf, QString p, QString fn)
{
    ...
    if( re.DIR_Name[0] != '\0' )
    {
        ...
        if( file.open(QIODevice::ReadOnly) )
        {
            ...
            {
                file.seek(rf.BPB_BytsPerSec * (33 + j - 2)); //减2是因为前两个扇区是保留的
...
int main(int argc, char *argv[])
{
    ...
    QString fn = "G:\\data.img";
    Fat12Header f12;

    qDebug() << "Read Header:";
    PrintHeader(f12, fn); //这里的路径设置很奇怪 增加了下一级目录就读不到了
    qDebug() << endl;

    qDebug() << "Print Root Entry:";

    PrintRootEntry(f12, fn);

    qDebug() << endl;

    qDebug() << "Print File Content:";

    QString content = QString(ReadFileContent(f12, fn, "TEST.TXT"));
